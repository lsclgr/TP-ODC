...
void createProgramExponential(double base, double exponent, double* RAM) {
    ...
    Instruction inst;
    double baseConvert = 0, result, realBase;
    ...
    createProgramRest((int)exponent, 2, RAM);
    ...
    inst.addressThree = -1;
    interpretedMachine(&inst, RAM);
    int rest = inst.addressOne;

    if (rest != 0 && baseConvert < 0) {
        createProgramSum(result, result, RAM);
