...
void createProgramMultiply(double multiplicand, double multiplier,
                           double* RAM) {
    // 0 => somar
    // 1 => sub
    ...
    Instruction* multiplyInstructions =
        malloc(((int)multiplier + 3) * sizeof(Instruction));
    ...
    if ((multiplicandConvert < 0 && multiplierConvert >= 0) ||
        (multiplicandConvert >= 0 && multiplierConvert < 0)) {
        createProgramSum(inst.addressOne, inst.addressOne, RAM);
...
void createProgramDivide(double dividend, double divisor, double* RAM) {
    ...
    if ((dividendConvert < 0 && divisorConvert >= 0) ||
        (dividendConvert >= 0 && divisorConvert < 0)) {
...
void createProgramRest(double dividend, double divisor, double* RAM) {
    Instruction inst;

    Instruction* restInstructions = malloc(6 * sizeof(Instruction));

    inst.opCode = 2;
    inst.addressOne = dividend;
    inst.addressTwo = 501;
    inst.addressThree = -1;
    restInstructions[0] = inst;
    // RAM[0] = dividend
    ...
    inst.addressTwo = 532;
    inst.addressThree = -1;
    restInstructions[2] = inst;

    inst.opCode = 2;
    inst.addressOne = 0;
    inst.addressTwo = 533;
    inst.addressThree = -1;
    restInstructions[3] = inst;

    inst.opCode = 2;
    inst.addressOne = dividend;
    inst.addressTwo = 534;
    inst.addressThree = -1;
    restInstructions[4] = inst;

    inst.opCode = -1;
    inst.addressOne = -1;
    inst.addressTwo = -1;
    inst.addressThree = -1;
    restInstructions[5] = inst;

    machine(restInstructions, RAM);
